...
int main(void)
{
	...
	ret = OSTaskCreate(&highest_prio, (void *)0, &highest_prio_stk[SIZE-1], 1);
	if(ret)
		printf("Highest priority task creation failed\n\r");
	printf("Highest prio task created\n\r");

	ret = OSTaskCreate(&medium_prio, (void *)0, &medium_prio_stk[SIZE-1], 2);
	if(ret)
		printf("Medium priority task creation failed\n\r");
	printf("Medium prio task created\n\r");

	ret = OSTaskCreate(&lowest_prio, (void *)0, &lowest_prio_stk[SIZE-1], 3);
	if(ret)
...
void lowest_prio(void *arg){
	...
	OSTaskChangePrio(OS_PRIO_SELF, 0);
	printf("Now Lowest prio task has prio %c\n\r", OS_PRIO_SELF);
	...
	for(;;){
		...
		if(i == 10){
			...
			OSSemPost(sem);
			printf("Deleting lowest prio task\n\r");
			OSTaskDel(OS_PRIO_SELF);
		}
	}
}
